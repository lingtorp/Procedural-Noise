...
// TODO: Add Doxygen style comments and documentation
/**
 * A collection of linear algebra data types mainly vectors and some of their operations.
 */
...
struct Vec3 {
    ...
    /// Operators
    inline Vec3<T> operator+(const Vec3 &rhs) const { return Vec3<T>{x + rhs.x, y + rhs.y, z + rhs.z}; }

    inline Vec3<T> operator+(const double rhs) const { return Vec3<T>{x + rhs, y + rhs, z + rhs}; }
