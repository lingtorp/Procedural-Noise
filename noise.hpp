...
public:
    ...
    virtual double get_value(const double x, const double y) const = 0;

    /// 3D raw noise from the underlying noise algorithm
    virtual double get_value(const double x, const double y, const double z) const = 0;
    ...
    double turbulence_ridged(const double x, const double y, const double z, const double zoom_factor) const {
      ...
      double zoom = zoom_factor;
        while (zoom >= 1.0) {
            value += (1.0 - std::abs(get_value(x / zoom, y / zoom, z / zoom) * zoom));
            zoom /= 2;
        }
        return value / zoom_factor;
    }
...
namespace Simplex {
    ...
    public:
        ...
        double get_value(const double x, const double y) const override {
          /// Skew
      ...
      double get_value(const double x, const double y, const double z) const override {
